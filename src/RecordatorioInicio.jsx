...
export default function RecordatorioInicio({ pedidos, onIrAVencidos, onIrAProximos }) {
  ...
  useEffect(() => {
    if (!yaHoy && (vencidos.length > 0 || proximos.length > 0)) {
      try { localStorage.setItem(LS_KEY, hoyStr()); } catch {}
    }
  }, [yaHoy, vencidos.length, proximos.length]);

  if (cerrado || yaHoy || (vencidos.length === 0 && proximos.length === 0)) return null;
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <div style={{ flex: 1 }}>
          <div style={{
            ...
            {vencidos.length > 0 ? "⏰ Hay pedidos vencidos" : "📌 Pedidos próximos a vencer"}
          </div>
