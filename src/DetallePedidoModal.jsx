...
export default function DetallePedidoModal({
  ...
  return (
    ...
    >
      ...
      {verVersiones && (
        <ModalVersiones pedido={pedido} onClose={() => setVerVersiones(false)} />
      )}
...
// Modal con lista de versiones anteriores del pedido y botón restaurar.
function ModalVersiones({ pedido, onClose }) {
  ...
  // Diff rápido: campos que cambian entre dos snapshots
  ...
  return (
    <Modal title={`🕗 Versiones de N°${String(pedido.id).padStart(4, "0")}`} onClose={onClose}>
      <div style={{ fontSize: 11, color: "#888", marginBottom: 10 }}>
        ...
        Click en una versión para restaurar.
      </div>
...
// snake_case → camelCase para comparar campos del snapshot (snake) con pedido del front (camel)
function toCamel(s) {
